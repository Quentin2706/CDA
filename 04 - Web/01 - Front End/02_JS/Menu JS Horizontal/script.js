...
/* Menu telephone / tablette */

var menuHamburger = document.getElementById("hamburger").parentNode;
var MenuTelFlag = false;
menuHamburger.addEventListener("click", OpenMenuTel);
...
function OpenMenuTel() {
    ...
    let icon = document.getElementById("hamburger");
        ...
        setTimeout(function(){ icon.classList.remove("shake"); }, 600);

    if (MenuTelFlag == false) {

        for (let i = 0; i < listeMenus.length; i++) {

            listeMenus[i].style.display = "block";
