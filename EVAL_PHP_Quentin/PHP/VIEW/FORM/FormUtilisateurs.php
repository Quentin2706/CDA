...
echo '<form class="GridForm" action="index.php?page=ActionUtilisateurs&mode='.$_GET['mode'].'&admin=true" method="post"/>';
...
echo '<label for=Nom class="caseForm labelForm">'."Nom".'</label>';
...
echo '<label for=Prenom class="caseForm labelForm">'."Prenom".'</label>';
echo '<div class="caseForm donneeForm"><input type="text" '.$disabled .'value="'.$elm->getPrenom().'" name=Prenom pattern="'.$regex["*"].'"></div>';
echo '<div class="caseForm infoForm"><i class="fas fa-question-circle"></i></div>';
echo '<div class="caseForm checkForm"><i class="fas fa-check-circle"></i></div>';

echo '<label for=AdresseMail class="caseForm labelForm">'."AdresseMail".'</label>';
